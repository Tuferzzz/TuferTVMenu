...
public class ChannelTuning extends MstarBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        viewholder_channeltune = new ViewHolder(this);
        ...
        if (isDtvAutoUpdateScan) {
            ...
            if (TvChannelManager.TV_ROUTE_DVBC == mCurrentRoute) {
                viewholder_channeltune.text_cha_tuningprogress_rf
                        .setText(getResources().getString(R.string.str_cha_tuningprogress_freq));
                ...
                int cableOperator = getIntent().getIntExtra("cableOperator", TvChannelManager.CABLE_OPERATOR_OTHER);
                int symbol = getIntent().getIntExtra("symbol", INVALID_SYMBOL);
                int modulation = getIntent().getIntExtra("modulation", DvbcScanParam.DVBC_CAB_TYPE_INVALID);
                ...
                mTvChannelManager.setDvbcScanParam((short)symbol,
                        modulation, frequency, frequency, (short) networkId);
                switch (scanType) {
    ...
    private void updateDtvTuningScanInfo(DtvEventScan extra) {
        ...
        if (TvChannelManager.DTV_SCAN_STATUS_AUTOTUNING_PROGRESS == scan_status) {
            ...
            if ((TvChannelManager.TV_ROUTE_DVBS == mCurrentRoute)
                ...
                if (((TvChannelManager.TV_ROUTE_DVBT == mCurrentRoute)
                        || (TvChannelManager.TV_ROUTE_DVBT2 == mCurrentRoute)
                        || (TvChannelManager.TV_ROUTE_DTMB == mCurrentRoute))
            ...
            TargetRegionListDialog countryDialog = new TargetRegionListDialog(this, Constant.TARGET_REGION_COUNTRY_LEVEL, countryNameList) {
                @Override
                public void onShow() {
                    if (View.VISIBLE == viewholder_channeltune.linear_cha_mainlinear.getVisibility()) {
                        ...
                        if (null != view) {
                            view.animate().alpha(0f)
                                    .setDuration(ChannelTuning.this.getResources().getInteger(android.R.integer.config_shortAnimTime));
                        }
                    }
                }
                @Override
                public void onDismiss() {
                    super.onDismiss();
                    showNextTargetRegionDialog(Constant.TARGET_REGION_COUNTRY_LEVEL, getSelectIndex());
                }
            };
            countryDialog.show();
        }
//        else if (TvChannelManager.STATUS_SET_REGIONAL_CHANNELLIST == scan_status) {
//            DvbRegionChannelListInfo mRegionChannelListInfo = TvDvbChannelManager.getInstance().getRegionChannellistInfo();
//            Log.d(TAG, "display region channel list info dialog, count = " + mRegionChannelListInfo.count);
...
//        }
    ...
    public void showNextTargetRegionDialog(int currentLevel, int currentSelectIndex) {
        Log.d(TAG, "showNextTargetRegionDialog: currentLevel = " + currentLevel + ", currentSelectIndex = " + currentSelectIndex);
        if (Constant.TARGET_REGION_COUNTRY_LEVEL == currentLevel) {
            ...
            if (countryInfo.primaryRegionInfos.length > 0) {
                ...
                TargetRegionListDialog primaryDialog = new TargetRegionListDialog(this, Constant.TARGET_REGION_PRIMARY_LEVEL, primaryNameList) {
                    @Override
                    public void onDismiss() {
                        super.onDismiss();
                        showNextTargetRegionDialog(Constant.TARGET_REGION_PRIMARY_LEVEL, getSelectIndex());
                    }
                };
                primaryDialog.show();
                return;
            ...
            if (primaryInfo.secondaryRegionInfos.length > 0) {
                ...
                TargetRegionListDialog secondaryDialog = new TargetRegionListDialog(this, Constant.TARGET_REGION_SECONDARY_LEVEL, secondaryNameList) {
                    @Override
                    public void onDismiss() {
                        super.onDismiss();
                        showNextTargetRegionDialog(Constant.TARGET_REGION_SECONDARY_LEVEL, getSelectIndex());
                    }
                };
            ...
            if (secondaryInfo.tertiaryRegionInfos.length > 0) {
                ...
                TargetRegionListDialog tertiaryDialog = new TargetRegionListDialog(this, Constant.TARGET_REGION_TERTIARY_LEVEL, tertiaryNameList) {
                    @Override
                    public void onDismiss() {
                        super.onDismiss();
                        showNextTargetRegionDialog(Constant.TARGET_REGION_TERTIARY_LEVEL, getSelectIndex());
                    }
                };
                tertiaryDialog.show();
                return;
        ...
        TvDvbChannelManager.getInstance().setRegionInfo(mTargetRegionCountryCode, mTargetRegionPrimaryCode, mTargetRegionSecondaryCode, mTargetRegionTertiaryCode);
